...
from trainer.pipelines.validators import VerificationResult
...
class PipelineStage(ABC):
    ...
    def verify_input(self, batch: ForwardBatch,
                     trainer_args: TrainerArgs) -> VerificationResult:
        """
        Verify the input for the stage.

        Example:
            from trainer.pipelines.stages.validators import V, VerificationResult
            
            def verify_input(self, batch, trainer_args):
    ...
    def __call__(
        ...
        enable_verification = getattr(trainer_args,
                                      'enable_stage_verification', False)

        if enable_verification:
            # Pre-execution input verification
            try:
                input_result = self.verify_input(batch, trainer_args)
        ...
        if envs.TRAINER_STAGE_LOGGING:
            ...
            try:
                result = self.forward(batch, trainer_args)
        ...
        if enable_verification:
            ...
            try:
                output_result = self.verify_output(result, trainer_args)
