...
from trainer.platforms.interface import (AttentionBackendEnum,
...
class CudaPlatformBase(Platform):
    ...
    def get_attn_backend_cls(cls, selected_backend: AttentionBackendEnum | None,
        ...
        logger.info("Trying TRAINER_ATTENTION_BACKEND=%s",
                    envs.TRAINER_ATTENTION_BACKEND)
        if selected_backend == AttentionBackendEnum.SLIDING_TILE_ATTN:
            try:
                from st_attn import sliding_tile_attention  # noqa: F401

                from trainer.attention.backends.sliding_tile_attn import (  # noqa: F401
                    SlidingTileAttentionBackend)
                logger.info("Using Sliding Tile Attention backend.")

                return "trainer.attention.backends.sliding_tile_attn.SlidingTileAttentionBackend"
            ...
            try:
                ...
                from trainer.attention.backends.sage_attn import (  # noqa: F401
                    SageAttentionBackend)
                logger.info("Using Sage Attention backend.")

                return "trainer.attention.backends.sage_attn.SageAttentionBackend"
            except ImportError as e:
                logger.info(e)
                logger.info(
            ...
            try:
                ...
                from trainer.attention.backends.video_sparse_attn import (  # noqa: F401
                    VideoSparseAttentionBackend)
                logger.info("Using Video Sparse Attention backend.")

                return "trainer.attention.backends.video_sparse_attn.VideoSparseAttentionBackend"
            except ImportError as e:
                logger.error(
            ...
            return "trainer.attention.backends.sdpa.SDPABackend"
        ...
        if target_backend == AttentionBackendEnum.FLASH_ATTN:
            try:
                ...
                from trainer.attention.backends.flash_attn import (  # noqa: F401
        ...
        return "trainer.attention.backends.flash_attn.FlashAttentionBackend"

    @classmethod
    def get_device_communicator_cls(cls) -> str:
        return "trainer.distributed.device_communicators.cuda_communicator.CudaCommunicator"  # noqa
