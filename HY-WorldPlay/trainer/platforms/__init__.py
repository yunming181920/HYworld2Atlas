...
from trainer.platforms.interface import AttentionBackendEnum  # noqa: F401
from trainer.platforms.interface import Platform, PlatformEnum
from trainer.utils import resolve_obj_by_qualname
...
def cuda_platform_plugin() -> str | None:
    ...
    try:
        ...
        pynvml.nvmlInit()
        try:
            # NOTE: Edge case: trainer cpu build on a GPU machine.
            # Third-party pynvml can be imported in cpu build,
            # we need to check if trainer is built with cpu too.
            # Otherwise, trainer will always activate cuda plugin
    ...
    return "trainer.platforms.cuda.CudaPlatform" if is_cuda else None
...
def mps_platform_plugin() -> str | None:
    ...
    return "trainer.platforms.mps.MpsPlatform" if is_mps else None


def cpu_platform_plugin() -> str | None:
    """Detect if CPU platform should be used."""
    # CPU is always available as a fallback
    return "trainer.platforms.cpu.CpuPlatform"
...
def __getattr__(name: str):
    if name == 'current_platform':
        ...
        # 1. out-of-tree platform plugins need `from trainer.platforms import
        #    Platform` so that they can inherit `Platform` class. Therefore,
        #    we cannot resolve `current_platform` during the import of
        #    `trainer.platforms`.
        # 2. when users use out-of-tree platform plugins, they might run
        #    `import trainer`, some trainer internal code might access
