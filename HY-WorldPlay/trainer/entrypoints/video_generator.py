...
"""
VideoGenerator module for Trainer.
...
from trainer.utils import align_to, shallow_asdict
...
class VideoGenerator:
    ...
    def __init__(self, trainer_args: TrainerArgs,
        ...
        self.executor = executor_class(trainer_args)
    ...
    def from_pretrained(cls,
        ...
        Args:
            ...
            **kwargs: Additional arguments to customize model loading, set any TrainerArgs or PipelineConfig attributes here.
        ...
        """
        # If users also provide some kwargs, it will override the TrainerArgs and PipelineConfig.
        kwargs['model_path'] = model_path
        trainer_args = TrainerArgs.from_kwargs(**kwargs)

        return cls.from_trainer_args(trainer_args)

    @classmethod
    def from_trainer_args(cls,
                            trainer_args: TrainerArgs) -> "VideoGenerator":
        """
        ...
        # initialize_distributed_and_parallelism(trainer_args)

        executor_class = Executor.get_class(trainer_args)
        return cls(
    ...
    def generate_video(
        ...
        **kwargs,
        ...
        Args:
            ...
            negative_prompt: The negative prompt to use (overrides the one in trainer_args)
            output_path: Path to save the video (overrides the one in trainer_args)
            ...
            num_inference_steps: Number of denoising steps (overrides trainer_args)
            guidance_scale: Classifier-free guidance scale (overrides trainer_args)
            num_frames: Number of frames to generate (overrides trainer_args)
            height: Height of generated video (overrides trainer_args)
            width: Width of generated video (overrides trainer_args)
            fps: Frames per second for saved video (overrides trainer_args)
            seed: Random seed for generation (overrides trainer_args)
        ...
        if self.trainer_args.prompt_txt is not None:
            prompt_txt_path = self.trainer_args.prompt_txt
    ...
    def _generate_single_video(
        ...
        **kwargs,
        ...
        pipeline_config = trainer_args.pipeline_config
        ...
        num_gpus = trainer_args.num_gpus
        ...
        if orig_latent_num_frames % trainer_args.num_gpus != 0:
       ...
       num_videos_per_prompt: {sampling_param.num_videos_per_prompt}
              guidance_scale: {sampling_param.guidance_scale}
                    n_tokens: {n_tokens}
                  flow_shift: {trainer_args.pipeline_config.flow_shift}
     embedded_guidance_scale: {trainer_args.pipeline_config.embedded_cfg_scale}
        ...
        batch = ForwardBatch(
            **shallow_asdict(sampling_param),
            eta=0.0,
            n_tokens=n_tokens,
            VSA_sparsity=trainer_args.VSA_sparsity,
            extra={},
        )
        ...
        output_batch = self.executor.execute_forward(batch, trainer_args)
