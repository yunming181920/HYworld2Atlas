...
class DistWrapper(object):
    def __init__(self, pipe, dist_controller: DistController, config, chunk_dim) -> None:
        super().__init__()
        self.pipe = pipe
    ...
    def wanattention_plugin_mount(self):
        ...
        for module in self.pipe.encoder.named_modules():
            ...
            if ('mid_block.' in module[0] and module[1].__class__.__name__ == 'WanAttentionBlock'):
    ...
    def inference(
        ...
        local_pose_image,
        local_latents,
        #prompts="A beaglening diving goggles  swimming in the ocean while the camera is moving, coral reefs in the background",
        config={},
        pipe_configs={
            "steps": 50,
            "guidance_scale": 12,
