...
def distribute_data_to_gpus(data, dim, rank, num_gpus, dtype):
    dim_size = data.shape[dim]
    assert dim_size % num_gpus == 0, f"Dim {dim_size} of data is not a multiply of {num_gpus}"
    ...
    local_data = data[slices].to(device=f'cuda:{rank}', dtype=dtype)
    print(f"Rank {rank} local_data: {local_data.shape}")
    return local_data

def gather_results(local_result, rank, dim, world_size):
    dist.barrier()
