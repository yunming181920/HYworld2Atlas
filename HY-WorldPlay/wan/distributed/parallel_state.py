...
import distributed.envs as envs
from distributed.device_communicators.base_device_communicator import (
    DeviceCommunicatorBase,
)
from distributed.device_communicators.cpu_communicator import CpuCommunicator
from distributed.utils import StatelessProcessGroup
from distributed.logger import init_logger
    ...
    def __init__(
        ...
        if use_device_communicator and self.world_size > 1:
            ...
            if current_platform.is_cuda_alike():
                from distributed.device_communicators.cuda_communicator import (
                    CudaCommunicator,
                )
...
def initialize_model_parallel(
    ...
    ), f"world_size({world_size}) < tensor_model_parallel_size({tensor_model_parallel_size})"
...
def get_sp_parallel_local_rank() -> int:
    """Return my local rank for the sequence model parallel group."""
    return get_sp_group().local_rank
...
def maybe_init_distributed_environment_and_model_parallel(
    ...
    initialize_model_parallel(
        tensor_model_parallel_size=tp_size, sequence_model_parallel_size=sp_size
    )
