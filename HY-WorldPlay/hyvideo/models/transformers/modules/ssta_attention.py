...
def similarity_sampling(
    q, k, topk, threshold=0.0, block_num=None, adaptive_pool=None, temperature=0.01
):
    ...
    if threshold > 0.0:
        ...
        dynamic_topk = torch.where(
            has_any_above, dynamic_topk, torch.full_like(dynamic_topk, topk)
        )
        ...
        top_block_indices = (
            torch.gather(sorted_indices, -1, indices) * mask
...
def create_moba_3d_mask(
    ...
    def get_block_avg_feat(
        x: torch.Tensor, adaptive_pool=None, pooling_type="avg"
    ) -> torch.Tensor:
        ...
        if adaptive_pool is not None:
            ...
            if pooling_type == "avg":
                global_avg = torch.nn.functional.adaptive_avg_pool3d(
                    x_block_means, adaptive_pool
                )
                ...
                max_pool = torch.nn.functional.adaptive_max_pool3d(
                    x_block_means, adaptive_pool
                )
    ...
    k_block_means = k_block_means.type(
        torch.float32
    ...
    top_block_indices = top_block_indices.squeeze(
        0
    )  # Remove batch dimension [h, s_block, topk]
    ...
    for head_idx in range(top_block_indices.size(0)):
        ...
        head_mask = torch.zeros(
            (block_num, block_num), dtype=torch.bool, device=q.device
        )
...
def create_sta_3d_mask_optimize(canvas_thw, tile_thw, kernel_thw):
    ...
    kernel_center_t = np.clip(
        q_t_tile, kernel_t // 2, (tile_thw_num[0] - 1) - kernel_t // 2
    )
    kernel_center_h = np.clip(
        q_h_tile, kernel_h // 2, (tile_thw_num[1] - 1) - kernel_h // 2
    )
    kernel_center_w = np.clip(
        q_w_tile, kernel_w // 2, (tile_thw_num[2] - 1) - kernel_w // 2
    )
...
def create_ssta_3d_mask(
    ...
    if text_mask is not None:
        ...
        assert (
            ssta_3d_mask.shape[-1] == ssta_3d_mask.shape[-2]
        ), "ssta_3d_mask should be square"
...
def ssta_3d_attention(
    ...
    assert (
        lambda_ is not None and sampling_type == "importance"
    ) or sampling_type == "similarity"
    ...
    if sparse_type == "sta":
        ...
        block_mask = create_sta_3d_mask(
            canvas_thw, tile_thw, kernel_thw, text_block_num
        ).to(q.device)
