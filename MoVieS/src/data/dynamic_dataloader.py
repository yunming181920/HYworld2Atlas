# Modified from https://github.com/facebookresearch/vggt/blob/main/training/data/dynamic_dataloader.py
...
class DynamicDataLoader:
    def __init__(self,
        ...
        if opt.random_video_size:
            self.aspect_ratio_range = opt.aspect_ratio_range
            self.image_num_range = (opt.min_num_input_frames, opt.num_input_frames)
        else:
            self.aspect_ratio_range = (1., 1.)
            self.image_num_range = (opt.num_input_frames, opt.num_input_frames)
        ...
        self.batch_sampler = DynamicBatchSampler(
            len(self.dataset) // (max_img_per_gpu // opt.num_input_frames),  # number of iterations per epoch
            self.sampler,
...
class DynamicDistributedSampler(DistributedSampler):
    ...
    def __iter__(self):
        ...
        for idx in indices_iter:
            yield (idx, self.aspect_ratio, self.image_num,)
